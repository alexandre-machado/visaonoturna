...
<%
'  ** Copyright 1999-2004 by John Martin d/b/a www.ANYPORTAL.com  **
'  ** All Rights Reserved.                                        **
'  **                                                             **
...
'  ** 12 JUN 04  add upload script from www.stardeveloper.com     **
...
'  ** 12 JUN 04  Add optional hide script from listing            **
	...
	' security
	Dim gblPassword
	gblPassword = "xxx"	'your password here
...
'--
' upload script developed by www.stardeveloper.com
	...
	Else
		...
		Case "UPLOAD"
			...
			Case "Script"
				...
				Set load = new Loader		
				load.initialize		
				fileData = load.getFileData("F1")
				fileName = LCase(load.getFileName("F1"))
				filePath = load.getFilePath("F1")
				filePathComplete = load.getFilePathComplete("F1")
				fileSize = load.getFileSize("F1")
				fileSizeTranslated = load.getFileSizeTranslated("F1")
				contentType = load.getContentType("F1")
				countElements = load.Count
				pathToFile = fsdir & "\" & fileName
				fileUploaded = load.saveToFile("F1",pathToFile)
				Set load = Nothing
			Case Else
			End Select
		Case "SAVE"
