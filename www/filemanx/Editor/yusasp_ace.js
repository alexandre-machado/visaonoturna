		...
		if(oEl)
			...
			if(inpAnchor!="")
				...
				if(isImgAnchorExist!=true)
					oEl.innerHTML = "<IMG src='editor/yusasp_anchor.gif' align=middle border=0>" + oEl.innerHTML;
				}
			else
				{
				for(var j=0;j<oEl.childNodes.length;j++)
	...
	for(var i=0;i<oEditor.document.all.length;i++)
		...
		if(oEditor.document.all[i].tagName=="A")
			...
			if(oEditor.document.all[i].name!="")
				oEditor.document.all[i].innerHTML = "<IMG src='editor/yusasp_anchor.gif' align=middle border=0>" + oEditor.document.all[i].innerHTML;
			}
		}
	//~~~~~~~~~~~~~
	}
...
function ACEditor(oName)
	...
	this.iconFolder = "editor/default/";
	...
	this.ImagePageURL = "";
	this.ImagePageWidth = 380;
	this.ImagePageHeight = 170;
